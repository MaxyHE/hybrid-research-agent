...
def api_get_available_models():
    ...
    try:
        ...
        if parse_bool_arg("configured_only") and not force_refresh:
            provider = normalize_provider(
                _get_setting_from_session("llm.provider", "ollama")
            )
            model = _get_setting_from_session("llm.model", "")
            models = (
                [{"value": model, "label": model, "provider": provider.upper()}]
                if model else []
            )
            return jsonify(
                provider_options=provider_options,
                providers={f"{provider}_models": models},
            )
