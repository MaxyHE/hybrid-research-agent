...
def get_agent_console_config() -> dict:
    """Return the public, deployment-safe Agent Console capability summary.

    Deliberately expose only booleans and positive integer budgets. Collection
    ids, endpoint URLs, model routing details, trace paths, and credentials stay
    server-side.
    """
    ...
    return {
        "source_pack": {
            "id": "medical_public_sources_v1",
            "label": "Medical Public Sources v1",
            "provider": "MedlinePlus",
            "snapshot": "2026-06-30",
            "topics": 1017,
            "chunks": 5618,
            "configured": bool(
                os.getenv("LDR_HYBRID_COLLECTION_ENGINE", "").strip()
            ),
        },
        "query_rewrite": {
            "label": "Query-Rewrite SFT v4",
            "configured": bool(
                os.getenv(
                    "LDR_COLLECTION_QUERY_NORMALIZER_ENDPOINT", ""
                ).strip()
                and os.getenv(
                    "LDR_COLLECTION_QUERY_NORMALIZER_MODEL", ""
                ).strip()
            ),
        },
        "budgets": {
            "model_calls": _positive_int_env("LDR_AGENT_MAX_MODEL_CALLS"),
            "tool_calls": _positive_int_env("LDR_AGENT_MAX_TOOL_CALLS"),
        },
        "trace_enabled": bool(os.getenv("LDR_AGENT_TRACE_DIR", "").strip()),
    }
...
@research_bp.route("/evidence-brief/gaia")
@login_required
def gaia_evidence_brief_page():
    """Render the fixed, audited GAIA Hybrid product showcase."""
    from ..services.evidence_brief_showcase_service import (
        EvidenceBriefShowcaseUnavailable,
        get_evidence_brief_showcase,
    )

    try:
        brief = get_evidence_brief_showcase("gaia-hybrid-r3")
    except EvidenceBriefShowcaseUnavailable as exc:
        return render_template_with_defaults(
            "pages/evidence_brief_unavailable.html", message=str(exc)
        ), 503
    return render_template_with_defaults("pages/evidence_brief.html", brief=brief)
...
def _extract_research_params(data, settings_manager):
    ...
    questions_per_iteration, strategy, and agent-profile options.
    """
    ...
    # ``general`` is an independent evidence-first runner.  It must never be
    # routed through the legacy LangGraph implementation, even when the
    # requested source happens to be public web only.
    #
    # ``hybrid`` is a deliberately bounded product profile rather than a
    # second, hand-wired research pipeline. It selects the existing
    # LangGraph agent and gives it the public web primary plus the curated
    # native Collection RAG engine configured for this deployment. Keeping
    # the collection id in an environment variable means the public web
    # application never needs to know a demo database path or credentials.
    agent_research_profile = (
        str(data.get("agent_research_profile", "default")).strip().lower()
    )
    if agent_research_profile not in {"default", "general", "hybrid", "hybrid_odr"}:
        raise ValueError(
            "Unknown evidence profile. Choose standard, general evidence-first, "
            "or hybrid evidence."
        )

    # A General collection is a user-authorized capability, not a free-form
    # search-engine name.  The request boundary validates its opaque shape;
    # ownership, agent_enabled, and is_public are resolved twice (here for a
    # useful 4xx and again in the worker immediately before model exposure).
    raw_general_collection_id = data.get("general_collection_id")
    general_collection_id = None
    if raw_general_collection_id not in (None, ""):
        if agent_research_profile != "general":
            raise ValueError(
                "general_collection_id is available only in general evidence mode."
            )
        if not isinstance(raw_general_collection_id, str):
            raise ValueError("general_collection_id must be a UUID string.")
        try:
            general_collection_id = str(UUID(raw_general_collection_id.strip()))
        except ValueError as exc:
            raise ValueError("general_collection_id must be a UUID string.") from exc

    raw_hybrid_odr_collection_id = data.get("hybrid_odr_collection_id")
    hybrid_odr_collection_id = None
    if raw_hybrid_odr_collection_id not in (None, ""):
        if agent_research_profile != "hybrid_odr":
            raise ValueError(
                "hybrid_odr_collection_id is available only in Hybrid-ODR mode."
            )
        if not isinstance(raw_hybrid_odr_collection_id, str):
            raise ValueError("hybrid_odr_collection_id must be a UUID string.")
        try:
            hybrid_odr_collection_id = str(UUID(raw_hybrid_odr_collection_id.strip()))
        except ValueError as exc:
            raise ValueError(
                "hybrid_odr_collection_id must be a UUID string."
            ) from exc

    hybrid_odr_source_mode = None
    if agent_research_profile == "hybrid_odr":
        hybrid_odr_source_mode = data.get("hybrid_odr_source_mode") or (
            "hybrid_available" if hybrid_odr_collection_id else "web_only"
        )
        if hybrid_odr_source_mode not in ("web_only", "collection_only", "hybrid_available"):
            raise ValueError("Choose Web, Collection-only, or Hybrid research.")
        if hybrid_odr_source_mode == "web_only":
            hybrid_odr_collection_id = None
        elif not hybrid_odr_collection_id:
            raise ValueError("Select a Collection for this research mode.")

    agent_search_engine_allowlist = None
    agent_include_sub_research = None
    agent_query_rewrite_enabled = False
    if agent_research_profile == "general":
        strategy = "general-v1"
    elif agent_research_profile == "hybrid":
        ...
        if not collection_engine:
            raise ValueError(
                ...
                "Set LDR_HYBRID_COLLECTION_ENGINE before starting the web app."
            )
        strategy = "langgraph-agent"
        agent_search_engine_allowlist = [collection_engine]
        # A one-shot demo should make its evidence path inspectable and cost
        # bounded; nested subagents add noisy parallelism without strengthening
        # the Web + Collection + Fetch demonstration.
        agent_include_sub_research = False
        requested_rewrite = data.get("agent_query_rewrite_enabled", True)
        if not isinstance(requested_rewrite, bool):
            raise ValueError("Query-Rewrite SFT v4 must be on or off.")
        agent_query_rewrite_enabled = requested_rewrite
        if agent_query_rewrite_enabled and not (
            os.getenv("LDR_COLLECTION_QUERY_NORMALIZER_ENDPOINT", "").strip()
            and os.getenv("LDR_COLLECTION_QUERY_NORMALIZER_MODEL", "").strip()
        ):
            raise ValueError(
                "Query-Rewrite SFT v4 is enabled for this run but is not "
    ...
    return {
        ...
        "general_collection_id": general_collection_id,
        "hybrid_odr_collection_id": hybrid_odr_collection_id,
        "hybrid_odr_source_mode": hybrid_odr_source_mode,
...
def _precheck_engine_policy(settings_manager, params, search_engine, username):
    ...
    try:
        ...
        general_collection_id = params.get("general_collection_id")
        if general_collection_id:
            # This is an authorization check, not merely a UUID parse.  Do it
            # before policy construction so a private collection becomes the
            # run primary and forces local inference under adaptive scope.
                    ...
                    jsonify(
                        {
                            "status": "error",
                            "message": (
                                "The requested General collection is unavailable "
                                "or not authorized for agent research."
                            ),
                        }
                    ),
                    400,
        ...
        # General private-collection runs intentionally do not bind a public
        # Web connector.  A public collection supplements Web, so both its
        # collection engine and the chosen Web engine must pass the same
        # frozen run policy.  All other profiles retain the historical one
        # actual-engine check.
        if params.get("hybrid_odr_source_mode") == "collection_only":
            engines_to_validate = (f"collection_{params['hybrid_odr_collection_id']}",)
        elif general_collection is None:
            engines_to_validate = (search_engine,)
        else:
            collection_engine = f"collection_{general_collection.collection_id}"
            engines_to_validate = (
                (collection_engine, search_engine)
                if general_collection.is_public
                else (collection_engine,)
            )
        for engine_name in engines_to_validate:
                ...
                jsonify(
                    {
                        ...
                        "message": denial_guidance(
                            decision.reason,
                            target=f"Search engine '{engine_name}'",
...
def _apply_policy_overrides(settings_snapshot, params):
    ...
    # General collection visibility decides whether the run may expose public
    # Web or must stay local.  Make the authorized collection the immutable
    # policy primary before the worker builds its EgressContext / LLM; the
    # worker re-resolves the collection and repeats this assignment to close
    # request-to-worker races.  Other profiles retain the selected engine.
    general_collection_id = params.get("general_collection_id")
    if (
        params.get("agent_research_profile") == "general"
        and isinstance(general_collection_id, str)
        and general_collection_id
    ):
        settings_snapshot["search.tool"] = f"collection_{general_collection_id}"
    elif (
        params.get("agent_research_profile") == "hybrid_odr"
        and params.get("hybrid_odr_source_mode") == "collection_only"
    ):
        settings_snapshot["search.tool"] = f"collection_{params['hybrid_odr_collection_id']}"
    elif params.get("search_engine"):
        settings_snapshot["search.tool"] = params["search_engine"]
...
def start_research():
    ...
    with get_user_db_session(username) as db_session:
        ...
        questions_per_iteration = params["questions_per_iteration"]
        strategy = params["strategy"]
        agent_research_profile = params["agent_research_profile"]
        agent_search_engine_allowlist = params["agent_search_engine_allowlist"]
        agent_include_sub_research = params["agent_include_sub_research"]
        agent_query_rewrite_enabled = params["agent_query_rewrite_enabled"]
        general_collection_id = params["general_collection_id"]
        hybrid_odr_collection_id = params["hybrid_odr_collection_id"]
        hybrid_odr_source_mode = params["hybrid_odr_source_mode"]
    ...
    research_settings = {
        ...
        "submission": {
            ...
            "agent_research_profile": agent_research_profile,
            "agent_query_rewrite_enabled": agent_query_rewrite_enabled,
            # Opaque ID only. The worker re-authorizes it and keeps the
            # collection name/raw text out of public run metadata.
            "general_collection_id": general_collection_id,
            "hybrid_odr_collection_id": hybrid_odr_collection_id,
            "hybrid_odr_source_mode": hybrid_odr_source_mode,
        },
    ...
    try:
        with get_user_db_session(username) as db_session:
            ...
            sources = get_research_source_links_batch(
                [research.id], db_session, limit=None, include_source_type=True
            ).get(research.id, [])
