...
class HTMLDownloader(BaseDownloader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Static requests identify the project rather than impersonating a
        # browser.  Besides being more transparent to source operators, this
        # avoids sites that route a bare browser-shaped requests client to an
        # anti-abuse page.  The Playwright path supplies BROWSER_USER_AGENT
        # only when a real browser is actually used.
    ...
    def download_with_result(
        ...
        try:
            ...
            if not text.strip():
                return DownloadResult(
                    skip_reason="Extracted content is empty",
                    fetch_metadata=getattr(
                        self, "_last_fetch_metadata", None
                    ),
                )
            ...
            return DownloadResult(
                ...
                is_success=True,
                fetch_metadata=getattr(self, "_last_fetch_metadata", None),
            )
            ...
            return DownloadResult(
                ...
                fetch_metadata={
                    "retrieval_method": "direct_html",
                    "failure_code": "html_download_exception",
                    "exception_type": type(e).__name__,
                    "error": sanitize_error_for_client(str(e)),
                },
            )
    ...
    def _fetch_html(self, url: str) -> Optional[str]:
        ...
        self._last_fetch_metadata = {
            "retrieval_method": "direct_html",
            # Keep enough routing evidence to diagnose redirects without
            # retaining path/query/userinfo from an observed URL.
            "requested_origin": redact_url_for_log(url),
        }
        try:
            ...
            self._last_fetch_metadata.update(
                {
                    "http_status": response.status_code,
                    "final_origin": redact_url_for_log(response.url),
            ...
            if response.status_code == 200:
                ...
                self._last_fetch_metadata["failure_code"] = (
                    "unexpected_content_type"
                )
                return None
            logger.warning(f"HTTP {response.status_code} fetching {url}")
            self._last_fetch_metadata["failure_code"] = "http_status"
            self.rate_tracker.record_outcome(
                engine_type=engine_type,
                wait_time=wait_time,
                success=False,
