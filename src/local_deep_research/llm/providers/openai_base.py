...
class OpenAICompatibleProvider(BaseLLMProvider):
    ...
    # ``langchain-openai`` translates its ``max_tokens`` argument to the
    # newer OpenAI ``max_completion_tokens`` wire field. Some otherwise
    # OpenAI-compatible APIs still accept only the legacy ``max_tokens``
    # field, so subclasses can put that exact provider-specific field in
    # ``extra_body`` instead.
    requires_legacy_max_tokens_field = False

    @classmethod
    def _apply_max_tokens(cls, llm_params, max_tokens):
        if cls.requires_legacy_max_tokens_field:
            llm_params.setdefault("extra_body", {})["max_tokens"] = max_tokens
        else:
            llm_params["max_tokens"] = max_tokens

    @classmethod
    def _provider_extra_body(cls, settings_snapshot):
        """Return explicitly configured provider-specific wire parameters."""

        return {}

    @classmethod
    def _apply_provider_extra_body(cls, llm_params, settings_snapshot):
        extra_body = cls._provider_extra_body(settings_snapshot)
        if extra_body:
            llm_params.setdefault("extra_body", {}).update(extra_body)
    ...
    def create_llm(cls, model_name=None, temperature=0.7, **kwargs):
        ...
        try:
            ...
            if max_tokens:  # Treat 0 as unset (matches legacy behavior)
                cls._apply_max_tokens(llm_params, max_tokens)
        except NoSettingsContextError:
            pass  # Optional parameter
        cls._apply_provider_extra_body(llm_params, settings_snapshot)
        ...
        # Opaque task ID for correlating client lifecycle sidecars with local
        # OpenAI-compatible server access logs.  It contains no user content
        # or credential and is absent outside an instrumented request.
        from ...agent_harness.request_lifecycle import lifecycle_headers

        if headers := lifecycle_headers():
            llm_params["default_headers"] = dict(headers)
