...
# The Jina Reader fallback is the one intentionally trusted *egress* relay.
# This is an exact authority, not a suffix/pattern: a request to this host is
# sent through a user-configured HTTP(S) proxy in some Runtime deployments,
# where local DNS can be synthetic (for example a Teredo answer) even though
# the proxy can safely connect to the real host.  Do not add other hosts here.
TRUSTED_JINA_MIRROR_HOST = "r.jina.ai"


def is_trusted_jina_mirror_url(url: object) -> bool:
    """Return true only for the canonical Jina Reader HTTPS authority.

    ``netloc`` equality deliberately rules out userinfo, explicit ports,
    suffixes, and trailing-dot aliases.  The session's trusted-egress mode
    uses this only to avoid *local* pre-resolution; it is not a general URL
    allowlist and it does not validate any reader target embedded in the path.
    """
    ...
    return (
        parsed.scheme == "https"
        and parsed.netloc == TRUSTED_JINA_MIRROR_HOST
        and parsed.hostname == TRUSTED_JINA_MIRROR_HOST
...
class SafeSession(requests.Session):
    ...
    def __init__(
        ...
        allow_localhost: bool = False,
        allow_private_ips: bool = False,
        trusted_egress_host: Optional[str] = None,
    ):
        """
        ...
        Args:
            ...
            allow_private_ips: Whether to allow all private/internal IPs plus localhost.
                ...
                AlibabaCloud / Tencent / ECS) are ALWAYS blocked — see
                ``ssrf_validator.ALWAYS_BLOCKED_METADATA_IPS``.
            trusted_egress_host: Internal-only exact-host carve-out for the
                fixed Jina Reader relay.  It accepts only
                ``"r.jina.ai"``.  Every request (including redirects) from
                such a session must remain at exactly
                ``https://r.jina.ai``; this avoids local DNS pre-resolution
                when a configured proxy supplies synthetic DNS answers.
        """
        if trusted_egress_host not in (None, TRUSTED_JINA_MIRROR_HOST):
            raise ValueError("Unsupported trusted egress host")
        super().__init__()
        self.max_redirects = _MAX_REDIRECTS
        self.allow_localhost = allow_localhost
        self.allow_private_ips = allow_private_ips
        self.trusted_egress_host = trusted_egress_host

    def _validate_outbound_url(self, url: str) -> bool:
        """Validate an outgoing URL, applying the narrowly scoped relay rule.

        A trusted-egress session is deliberately more restrictive than a
        normal one: it cannot silently fall back to ordinary public egress on
        a redirect.  That prevents a trusted relay capability from becoming a
        general bypass merely because a relay response supplied ``Location``.
        """
        if self.trusted_egress_host is not None:
            if is_trusted_jina_mirror_url(url):
                return True
            raise ValueError(
                "Trusted egress session refused request outside "
                "https://r.jina.ai"
            )

        return ssrf_validator.validate_url(
            url,
    ...
    def request(self, method: str, url: str, **kwargs) -> requests.Response:  # type: ignore[override]
        ...
        if not self._validate_outbound_url(url):
            raise ValueError(
                f"URL failed security validation (possible SSRF): {url}"
            )
    ...
    def send(
        ...
        if request.url:
            try:
                is_allowed = self._validate_outbound_url(request.url)
            except ValueError:
                logger.warning(
            ...
            if not is_allowed:
                ...
                raise ValueError(
                    "Redirect target failed security validation "
                    f"(possible SSRF): {request.url}"
                )
