...
(function() {
    ...
    let generalCollectionChoicesLoaded = false;
    let generalCollectionChoicesLoading = false;
    ...
    /**
     * Keep the source-range control honest: the browser can choose whether a
     * selected local library is offered to Hybrid ODR, but the server remains
     * the authorization authority at submission and worker time.
     */
    async function syncGeneralCollectionControl() {
        const container = document.getElementById('general-collection-container');
        const select = document.getElementById('general_collection_id');
        const help = document.getElementById('general-collection-help');
        const summary = document.getElementById('research-source-summary');
        const selectedScope = document.querySelector('input[name="research_source_scope"]:checked');
        const scope = selectedScope ? selectedScope.value : 'web';
        if (!container || !select) return;

        document.querySelectorAll('.ldr-deep-source-option').forEach(option => {
            option.classList.toggle('is-selected', option.dataset.sourceScope === scope);
        });
        if (summary) {
            summary.textContent = scope === 'hybrid'
                ? '当前：混合研究 · 公开 Web + 本地文库'
                : scope === 'collection' ? '当前：仅本地文库' : '当前：仅公开 Web';
        }

        const usesCollection = scope === 'hybrid' || scope === 'collection';
        container.hidden = !usesCollection;
        if (!usesCollection) {
            select.value = '';
            select.disabled = true;
            return;
        }
        if (generalCollectionChoicesLoaded) {
            select.disabled = false;
            if (help && select.options.length > 1) {
                help.textContent = scope === 'collection'
                    ? '只查阅所选文库；本次研究不使用公开 Web。'
                    : '所选文库会与公开 Web 一起参与本次研究。';
            }
            return;
        }
        if (generalCollectionChoicesLoading) return;

        generalCollectionChoicesLoading = true;
        select.disabled = true;
        if (help) help.textContent = '正在载入可用于研究的本地文库…';
        try {
            const response = await fetch('/library/api/collections', {
                credentials: 'same-origin'
            });
            const payload = await response.json();
            if (!response.ok || !payload || payload.success !== true || !Array.isArray(payload.collections)) {
                throw new Error('collection list unavailable');
            }
            const enabled = payload.collections
                .filter(collection => collection && collection.agent_enabled === true)
                .sort((left, right) => String(left.name || '').localeCompare(String(right.name || '')));
            select.replaceChildren();
            const webOnly = document.createElement('option');
            webOnly.value = '';
            webOnly.textContent = '选择一个可用于研究的文库';
            select.appendChild(webOnly);
            for (const collection of enabled) {
                if (typeof collection.id !== 'string' || !collection.id) continue;
                const option = document.createElement('option');
                option.value = collection.id;
                option.textContent = String(collection.name || '未命名文库');
                select.appendChild(option);
            }
            generalCollectionChoicesLoaded = true;
            if (help) {
                help.textContent = enabled.length
                    ? '选择一个文库，按上方选择的来源范围参与本次研究。'
                    : '当前没有可用于研究的本地文库。你仍可仅使用公开 Web，或先在文库中启用一个集合。';
            }
        } catch (_error) {
            select.replaceChildren();
            const webOnly = document.createElement('option');
            webOnly.value = '';
            webOnly.textContent = '暂时无法载入本地文库';
            select.appendChild(webOnly);
            if (help) help.textContent = '本地文库暂时无法载入。请改用公开 Web，或稍后重试。';
        } finally {
            generalCollectionChoicesLoading = false;
            select.disabled = false;
        }
    }
    ...
    function initializeResearch() {
        ...
        // The saved choice is usable immediately; discovery must not blank it.
        ...
        Promise.all([
            ...
            searchEngineInput ? loadSearchEngineOptions(false) : Promise.resolve([])
    ...
    function setupEventListeners() {
        ...
        document.querySelectorAll('input[name="research_source_scope"]').forEach(input => {
            input.addEventListener('change', () => {
                void syncGeneralCollectionControl();
            });
        });
        void syncGeneralCollectionControl();
        ...
        return new Promise((resolve) => {
            ...
            const url = forceRefresh
                ...
                : URLS.SETTINGS_API.AVAILABLE_MODELS + (
                    document.getElementById('agent_research_profile')?.value === 'hybrid_odr'
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    if (data && data.providers) {
                        ...
                        // Cache before populating: population reads this cache.
    ...
    function handleResearchSubmit(event) {
        ...
        const selectedSourceScope = document.querySelector('input[name="research_source_scope"]:checked');
        const sourceScope = selectedSourceScope ? selectedSourceScope.value : 'web';
        const localLibrarySelect = document.getElementById('general_collection_id');
      ...
      // Use centralized security utilities for button update
        ...
        const evidenceProfileSelect = document.getElementById('agent_research_profile');
        const agentResearchProfile = evidenceProfileSelect ? evidenceProfileSelect.value : 'default';
        const generalCollectionSelect = document.getElementById('general_collection_id');
        const selectedCollectionId = sourceScope !== 'web'
            && agentResearchProfile === 'hybrid_odr'
            && generalCollectionSelect
            && typeof generalCollectionSelect.value === 'string'
            && generalCollectionSelect.value.trim()
            ? generalCollectionSelect.value.trim()
            : null;
        // Query-rewrite SFT is not a user-facing switch in Deep Research.
        // Keep the legacy request field inert for older backend contracts.
        const agentQueryRewriteEnabled = false;
        ...
        const formData = {
            ...
            agent_research_profile: agentResearchProfile,
            agent_query_rewrite_enabled: !!agentQueryRewriteEnabled,
            ...(agentResearchProfile !== 'hybrid_odr' ? {
                strategy, iterations, questions_per_iteration: questionsPerIteration
            } : {})
        };
        if (agentResearchProfile === 'hybrid_odr') {
            formData.hybrid_odr_source_mode = {
                web: 'web_only', collection: 'collection_only', hybrid: 'hybrid_available'
            }[sourceScope];
        }
        if (selectedCollectionId) {
            if (agentResearchProfile === 'general') {
                formData.general_collection_id = selectedCollectionId;
            } else {
                formData.hybrid_odr_collection_id = selectedCollectionId;
            }
        }
