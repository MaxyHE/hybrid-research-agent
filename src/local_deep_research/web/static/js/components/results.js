...
(function() {
    ...
    function hybridRunStatus(metadata) {
        const details = metadata && typeof metadata.hybrid_odr === 'object'
            ? metadata.hybrid_odr
            : null;
        if (!details) {
            return {
                className: '',
                title: '报告已生成',
                copy: '请结合报告中的引用与来源清单阅读本次研究。',
                metrics: ''
            };
        }
        const status = String(details.status || '').toLowerCase();
        const terminalReason = String(details.terminal_reason || '');
        const count = (value, label) => Number.isFinite(Number(value)) ? `${label} ${Number(value)}` : null;
        const metrics = [
            {web_only: '公开 Web', collection_only: '仅本地文库', hybrid_available: '混合研究'}[details.source_mode],
            count(details.task_count, '任务'),
            count(details.fetched_source_count, '来源'),
            count(details.model_calls_used, '模型调用'),
            count(details.tool_calls_used, '工具调用')
        ].filter(Boolean).join(' · ');
        if (status === 'complete') {
            return {
                className: 'is-complete',
                title: '研究完成',
                copy: terminalReason === 'citation_url_audit_passed'
                    ? '引用链接检查已通过。报告内容仍应结合来源自行判断。'
                    : '报告已生成。请结合引用与来源清单阅读本次研究。',
                metrics
            };
        }
        if (status === 'needs_review') {
            return {
                className: 'is-review',
                title: '引用需要复核',
                copy: '报告已生成，但引用链接需要人工复核；请先查看来源清单。',
                metrics
            };
        }
        if (status === 'incomplete') {
            return {
                ...
                title: '研究未完整完成',
                copy: '以下内容可能为部分结果。请查看来源，或重新发起研究。',
                metrics
            };
        }
        return {
    ...
    function renderRunStatus(data) {
        const panel = document.getElementById('research-run-status');
        const title = document.getElementById('research-run-status-title');
        const copy = document.getElementById('research-run-status-copy');
        const metrics = document.getElementById('research-run-metrics');
        const runId = document.getElementById('research-run-id');
        const state = hybridRunStatus(data && data.metadata);
        if (panel) panel.className = `ldr-deep-result-status ${state.className}`.trim();
        if (title) title.textContent = state.title;
        if (copy) copy.textContent = state.copy;
        if (metrics) metrics.textContent = state.metrics;
        if (runId) runId.textContent = researchId || '—';
    }

    function normaliseSource(source) {
        if (typeof source === 'string') {
            return { url: source, title: source, sourceType: 'web' };
        }
        if (!source || typeof source !== 'object' || typeof source.url !== 'string') return null;
        const url = source.url.trim();
        if (!url) return null;
        const sourceType = String(source.source_type || source.channel || 'web');
        return {
            url,
    ...
    function isSafeReportSourceUrl(url) {
        return /^https?:\/\//i.test(url) || /^\/library\/document\/[A-Za-z0-9][A-Za-z0-9_-]*(?:\/pdf)?$/.test(url);
    }

    function sourceLabel(source) {
        if (['local_collection', 'library', 'collection'].includes(source.sourceType) || /^\/library\/document\//.test(source.url)) {
            return '本地文库';
        }
        try {
            return new URL(source.url, window.location.origin).hostname || '公开 Web';
        } catch (_error) {
            return '公开 Web';
        }
    }

    function createSourceGroup(title, sources) {
        const group = document.createElement('section');
        group.className = 'ldr-deep-source-group';
        const heading = document.createElement('h3');
        heading.textContent = title;
        const list = document.createElement('ul');
        list.className = 'ldr-deep-source-list';
        for (const source of sources) {
            ...
            detail.textContent = sourceLabel(source);
    ...
    function renderSources(sources) {
        const panel = document.getElementById('research-sources-panel');
        const count = document.getElementById('research-sources-count');
        if (!panel) return;
        const safeSources = (Array.isArray(sources) ? sources : [])
            .map(normaliseSource)
            .filter(source => source && isSafeReportSourceUrl(source.url));
        panel.replaceChildren();
        if (count) count.textContent = `${safeSources.length} 项`;
        if (!safeSources.length) {
            const empty = document.createElement('p');
            empty.className = 'ldr-deep-sources-empty';
            empty.textContent = '本次报告没有可展示的来源链接。';
            panel.appendChild(empty);
            return;
        }
        const local = safeSources.filter(source => ['local_collection', 'library', 'collection'].includes(source.sourceType) || /^\/library\/document\//.test(source.url));
        const web = safeSources.filter(source => !local.includes(source));
        if (web.length) panel.appendChild(createSourceGroup('公开 Web', web));
        if (local.length) panel.appendChild(createSourceGroup('本地文库', local));
    }
    ...
    function setupEventListeners() {
        ...
        const rerunBtn = document.getElementById('rerun-research-btn');
        if (rerunBtn) {
            rerunBtn.addEventListener('click', () => {
                const query = researchData?.metadata?.query || researchData?.query;
                if (typeof query === 'string' && query.trim()) {
                    try {
                        sessionStorage.setItem('rerunConfig', JSON.stringify({ query, mode: 'quick' }));
                    } catch (error) {
    ...
    function handleExport() {
        try {
            ...
            const runStatus = document.getElementById('research-run-status-title')?.textContent || '';
            ...
            if (runStatus) markdownHeader += `- **Research status:** ${runStatus}\n`;
            markdownHeader += '\n';
            markdownHeader += `---\n\n`;
