...
# Hybrid ODR persists detailed run artifacts for server-side audit.  Browser
# report/status APIs deliberately project only these run-summary values; do not
# widen this list with artifact locations, traces, snapshots, or raw evidence.
_HYBRID_ODR_PUBLIC_METADATA_FIELDS = frozenset(
    {
        "status",
        "terminal_reason",
        "model_calls_used",
...
def strip_settings_snapshot(research_meta: Any) -> dict:
    """Remove private runtime metadata from research API responses.

    settings_snapshot contains all application settings including API keys.
    Hybrid ODR's detailed audit artifacts are also server-only. This helper
    preserves the existing general research metadata contract while applying
    an allowlist to the nested Hybrid ODR run summary.
    ...
    try:
        ...
        hybrid_odr = safe_meta.get("hybrid_odr")
        if isinstance(hybrid_odr, dict):
            safe_meta["hybrid_odr"] = {
                key: hybrid_odr[key]
                for key in _HYBRID_ODR_PUBLIC_METADATA_FIELDS
                if key in hybrid_odr
            }
        return safe_meta
    except (json.JSONDecodeError, TypeError, AttributeError):
        return {}
