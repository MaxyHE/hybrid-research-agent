...
(function() {
    ...
    function phaseLabel(phase) {
        const labels = {
            init: '正在准备本次研究',
            search_planning: '正在组织研究',
            output_generation: '正在整理报告',
            termination: '正在终止研究'
        };
        return labels[String(phase || '')] || null;
    }
    ...
    function updateProgressUI(data) {
        ...
        // Status is a direct mapping of the worker status; never infer tool
        // activity, researcher count, or verification from this page.
        if (data.status && statusText) {
            statusText.textContent = runStatusLabel(data.status);
        ...
        if (currentTaskText) {
            ...
            if (!taskMessage) {
                ...
                if (data.log_entry && data.log_entry.message && (data.log_entry.type === "milestone" || data.log_entry.type === "MILESTONE")) {
                    ...
                    taskMessage = phaseLabel(data.phase);
                    specificProgressMessage = true;
                } else {
            ...
            // Without a worker message we show only a generic status. We do
            // not fabricate searching, analysis, review, or tool-call stages.
            if (!specificProgressMessage && data.status &&
                (!currentTaskText.dataset.lastMessage || currentTaskText.textContent === 'In Progress')) {
                const statusMsg = data.status === window.RESEARCH_STATUS.QUEUED && data.queue_position
                    ? `等待运行（队列第 ${data.queue_position} 位）`
                    : '等待后端下一次状态更新…';
                if (statusMsg) {
    ...
    function handleResearchComplete(data) {
        ...
        // Show notification if enabled
        showNotification('研究完成', '报告已生成，可以查看研究结果。');
    ...
    function handleResearchError(data) {
        ...
        setCurrentTask(`运行未完成：${data.error || '未知错误'}`);
        ...
        if (viewResultsButton) {
            viewResultsButton.textContent = '查看可用结果';
            viewResultsButton.href = URLBuilder.resultsPage(currentResearchId);
            viewResultsButton.style.display = 'inline-block';
        }

        // Show notification if enabled
        showNotification('研究未完成', `运行发生错误：${data.error || '未知错误'}`);
    ...
    function setStatus(status) {
        ...
        statusText.textContent = runStatusLabel(status);
    }
    ...
    function setCurrentTask(task) {
        ...
        currentTaskText.textContent = task || '等待后端状态更新…';
    }
