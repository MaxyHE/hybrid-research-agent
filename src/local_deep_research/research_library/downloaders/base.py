...
class DownloadResult(NamedTuple):
    ...
    ``fetch_metadata`` carries transport facts (for example final HTTP
    status, redirect count, or a sanitized transport error) separately from
    the human-facing ``skip_reason``.  Callers can therefore diagnose a
    failed fetch without promoting an error page to evidence or leaking a
    credential-bearing URL through the normal tool result.
    """
