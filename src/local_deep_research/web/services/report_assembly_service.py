...
def _is_displayable_source_url(url: str) -> bool:
    """Return whether a persisted source URL is safe to expose as a link.

    Report pages can link to public HTTP(S) sources and to the application's
    document-detail route.  Filesystem paths, custom schemes, and arbitrary
    relative routes remain server-side-only values.
    """
    return url.startswith(("http://", "https://")) or bool(
        _LIBRARY_DOCUMENT_URL_RE.fullmatch(url)
    )
...
def get_research_source_links_batch(
    ...
    ``include_source_type`` is for that report API only so its UI can group
    public-Web and local-library sources without changing the news-card shape.
    """
    ...
    for r in rows:
        ...
        if limit is not None and len(bucket) >= limit:
            continue
        url = (r.url or "").strip()
        if not _is_displayable_source_url(url):
            continue
        title = (r.title or "").strip()
        if not title:
        ...
        if include_source_type:
            source["source_type"] = (r.source_type or "web").strip()
