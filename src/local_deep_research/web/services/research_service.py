...
class _GeneralReportSystemView:
    """Minimal persistence/streaming view for an audited General artifact.

    The surrounding web service historically expects a ``system`` object in
    order to read ``all_links_of_system`` while it stores a report.  General
    V1 must not construct that legacy strategy merely to satisfy this shape,
    so this view exposes only the non-agent source list and progress callback.
    It has no ``strategy`` attribute and therefore cannot accidentally enable
    legacy token streaming or synthesis.
    """

    def __init__(self) -> None:
        self.all_links_of_system: list[dict] = []
...
class _AuditedCitationPassthrough:
    """Preserve a General V1 renderer result during legacy persistence.

    General's writer assigns numeric citations deterministically and its
    citation audit has already checked every claim/citation relation.  The
    legacy formatter is designed for free-form LLM prose: it may split a
    ``Sources`` heading or transform brackets into hyperlinks.  Either action
    would make the stored report differ from the audited artifact.  This
    deliberately tiny compatibility object satisfies the existing persistence
    sequence without reparsing or rewriting the answer.
    """

    @staticmethod
    def format_document_split(content, *, trust_sentinel=False):
        del trust_sentinel
        return content, "", False

    @staticmethod
    def apply_inline_hyperlinks(content, _sources):
        return content
...
def run_research_process(research_id, query, mode, **kwargs):
    ...
    try:
        ...
        agent_research_profile = kwargs.get("agent_research_profile", "default")
        agent_search_engine_allowlist = kwargs.get(
            "agent_search_engine_allowlist"
        )
        agent_include_sub_research = kwargs.get("agent_include_sub_research")
        agent_query_rewrite_enabled = bool(
            kwargs.get("agent_query_rewrite_enabled", True)
        )
        if (
            agent_research_profile == "hybrid"
            and not agent_search_engine_allowlist
        ):
            collection_engine = os.getenv(
                "LDR_HYBRID_COLLECTION_ENGINE", ""
            ).strip()
            if not collection_engine:
                raise ValueError(
                    "Hybrid evidence mode is not configured on this server."
                )
            agent_search_engine_allowlist = [collection_engine]
            agent_include_sub_research = False
            strategy = "langgraph-agent"
        settings_snapshot = kwargs.get(
            "settings_snapshot", {}
        )  # Complete settings snapshot
        is_general_research = agent_research_profile == "general"
        is_hybrid_odr = agent_research_profile == "hybrid_odr"
        hybrid_odr_collection_id = kwargs.get("hybrid_odr_collection_id")
        hybrid_odr_source_mode = kwargs.get("hybrid_odr_source_mode") or (
            "hybrid_available" if hybrid_odr_collection_id else "web_only"
        )
        if is_hybrid_odr and hybrid_odr_source_mode == "collection_only":
            settings_snapshot = dict(settings_snapshot)
            settings_snapshot["search.tool"] = f"collection_{hybrid_odr_collection_id}"
        general_collection = None
        general_collection_id = kwargs.get("general_collection_id")
        if is_general_research:
            # ``general`` is dispatched below to GeneralResearchRunner.  Keep
            # its persisted strategy label honest even for queue/follow-up
            # paths that did not originate at the Flask request parser.
            strategy = "general-v1"
            if general_collection_id is not None:
                if not isinstance(general_collection_id, str) or not general_collection_id.strip():
                    raise ValueError("General collection id must be a UUID string.")
                from ...general_research.collection_connector import (
                    resolve_project_collection_descriptor,
                )

                # Re-authorize at the worker boundary.  A queued run must not
                # retain access merely because the collection was eligible
                # when it was submitted.
                general_collection = resolve_project_collection_descriptor(
                    collection_id=general_collection_id,
                    username=username,
                )
                if not isinstance(settings_snapshot, dict):
                    raise TypeError(
                        "General collection research requires a settings snapshot"
                    )
                # The collection is the immutable policy primary for both
                # public and private collection modes.  In adaptive scope the
                # collection's app-owned ``is_public`` metadata therefore
                # selects public vs. private inference before get_llm().
                settings_snapshot = dict(settings_snapshot)
                settings_snapshot["search.tool"] = (
                    f"collection_{general_collection.collection_id}"
                )
            if general_collection is None and not search_engine:
                from ...config.thread_settings import get_setting_from_snapshot

                search_engine = get_setting_from_snapshot(
                    "search.tool",
                    "searxng",
                    settings_snapshot=settings_snapshot,
                )
            requires_public_web = (
                general_collection is None or general_collection.is_public
            )
            if requires_public_web and (
                not isinstance(search_engine, str) or not search_engine.strip()
            ):
                raise ValueError(
                    "General research requires a configured public search engine."
                )
        ...
        logger.info(
            ...
            f"evidence_profile={agent_research_profile}"
        )
        ...
        try:
            ...
            # General private-collection mode never binds a public connector.
            # Its primary audit above is sufficient and forces local inference.
            # Web-only and public-collection modes do bind the named public
            # engine, so verify that exact secondary capability before any
            # connector or model call.  This covers programmatic/queued entry
            # points that bypass the HTTP precheck.
            if is_general_research and (
                general_collection is None or general_collection.is_public
            ):
                from ...security.egress.policy import evaluate_engine

                _public_engine = evaluate_engine(
                    search_engine,
                    _egress_ctx,
                    settings_snapshot=settings_snapshot,
                )
                if not _public_engine.allowed:
                    raise PolicyDeniedError(_public_engine, target=search_engine)
        except ImportError:
        ...
        def progress_callback(message, progress_percent, metadata):
            ...
            if mode == "detailed" and not is_general_research:
            ...
            if still_active:
                ...
                # Agent Console observability is event-based: dropping a
                # burst of parallel tool calls behind the generic 200ms
                ...
                if should_emit:
                    ...
                    if metadata.get("query_rewrite"):
                        event_data["query_rewrite"] = metadata["query_rewrite"]
                    if metadata.get("scheduled_tool_calls") is not None:
                        event_data["scheduled_tool_calls"] = metadata[
                            "scheduled_tool_calls"
                        ]
                    if metadata.get("max_tool_calls") is not None:
                        event_data["max_tool_calls"] = metadata[
                            "max_tool_calls"
                        ]
        ...
        if model or model_provider or is_general_research or is_hybrid_odr:
            try:
        ...
        use_search = None
        if search_engine and not (is_general_research or is_hybrid_odr):
            try:
        ...
        if is_general_research or is_hybrid_odr:
            system = _GeneralReportSystemView()
        else:
            system = AdvancedSearchSystem(
                ...
                research_id=research_id,
                research_context=shared_research_context,
                agent_search_engine_allowlist=agent_search_engine_allowlist,
                agent_research_profile=agent_research_profile,
                agent_include_sub_research=agent_include_sub_research,
                agent_query_rewrite_enabled=agent_query_rewrite_enabled,
            )
        ...
        try:
            if is_hybrid_odr:
                from .hybrid_odr_service import run_hybrid_odr

                progress_callback(
                    "Researching with the selected sources...",
                    20,
                    {"phase": "search_planning"},
                )
                hybrid_run = run_hybrid_odr(
                    run_id=research_id,
                    query=query,
                    llm=use_llm,
                    username=username,
                    settings_snapshot=settings_snapshot,
                    output_root=OUTPUT_DIR / "hybrid_odr",
                    collection_id=hybrid_odr_collection_id,
                    source_mode=hybrid_odr_source_mode,
                    user_password=user_password,
                ...
                results = {
                    "formatted_findings": hybrid_run.report_markdown,
                    "current_knowledge": hybrid_run.report_markdown,
                    "findings": [],
                    "iterations": max(1, hybrid_run.task_count),
                    "hybrid_odr_metadata": hybrid_run.metadata,
                    "final_markdown_is_final": True,
                }
                output_mode = "quick"
                progress_callback(
                    "Hybrid-ODR research finished; preparing report...",
                    85,
                    {"phase": "output_generation"},
                )
            elif is_general_research:
                # This is the sole General product entry point.  The General
                # adapter reuses configured LLM/search/egress primitives but
                # never calls AdvancedSearchSystem or any Hybrid fallback.
                from .general_research_service import run_general_research
                from ...general_research.workflow import GeneralResearchCancelled

                progress_callback(
                    "Planning and verifying evidence with General Research Agent...",
                    20,
                    {"phase": "search_planning"},
                )

                def _forward_general_progress(progress) -> None:
                    """Forward only the adapter's browser-safe event projection."""

                    progress_callback(
                        progress.message,
                        progress.progress_percent,
                        dict(progress.metadata) | {"phase": progress.phase},
                    )

                try:
                    general_run = run_general_research(
                        run_id=research_id,
                        query=query,
                        llm=use_llm,
                        search_engine_name=(
                            ...
                            or general_collection.is_public
                            else None
                        ),
                        settings_snapshot=settings_snapshot,
                        username=username,
                        ...
                        should_cancel=lambda: is_termination_requested(research_id),
                        on_progress=_forward_general_progress,
                    )
                except GeneralResearchCancelled as exc:
                    raise ResearchTerminatedException(
                        "Research was terminated during General research"
                    ) from exc
                system.all_links_of_system = list(general_run.source_rows)
                general_iterations = max(
                    1,
                    sum(
                        event.kind.value == "coverage"
                        for event in general_run.workflow_result.events
                    ),
                )
                results = {
                    # General renderer has already made and audited every
                    # factual claim.  The legacy persistence path consumes
                    # this answer-only body and must not re-synthesize it.
                    "formatted_findings": general_run.report_body_markdown,
                    "current_knowledge": general_run.report_body_markdown,
                    "findings": [],
                    "iterations": general_iterations,
                    "general_metadata": general_run.metadata,
                    "general_publishable": general_run.is_publishable,
                    # The final markdown is a deterministic rendering of the
                    # audited General document.  The legacy persistence code
                    # must preserve its exact citation text.
                    "general_output_is_final": True,
                    "final_markdown_is_final": True,
                }
                output_mode = "quick"
                progress_callback(
                    (
                        "Evidence audit passed; preparing report..."
                        if general_run.is_publishable
                        else "Evidence audit is incomplete; saving the explicit coverage result..."
                    ),
                    85,
                    {"phase": "output_generation"},
                )
            ...
            # General's cooperative cancellation signal uses the same
            # lifecycle exception as existing strategies, so the outer
            # handler marks the row SUSPENDED instead of persisting a partial
            # report as an execution failure.
            raise
        ...
        if output_mode == "quick":
            ...
            if results.get("findings") or results.get("formatted_findings"):
                ...
                try:
                    ...
                    formatter = (
                        _AuditedCitationPassthrough()
                        if results.get("final_markdown_is_final") is True
                        else get_citation_formatter()
                    )
                    try:
                    ...
                    with get_user_db_session(username) as db_session:
                        ...
                        if isinstance(results.get("general_metadata"), dict):
                            metadata.update(results["general_metadata"])
                        if isinstance(results.get("hybrid_odr_metadata"), dict):
                            metadata.update(results["hybrid_odr_metadata"])
                    ...
                    progress_callback(
                        (
                            ...
                            if results.get("general_publishable", True)
                            else "Research run completed, but the evidence gate withheld a final answer"
                        ),
                        100,
                        {"phase": "complete"},
                    )
