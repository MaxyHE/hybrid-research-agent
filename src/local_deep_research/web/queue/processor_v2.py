    ...
    def _start_research(
        ...
        try:
            research_thread = start_research_process(
                ...
                agent_research_profile=submission_params.get(
                    "agent_research_profile", "default"
                ),
                agent_query_rewrite_enabled=bool(
                    submission_params.get("agent_query_rewrite_enabled", True)
                ),
                general_collection_id=submission_params.get("general_collection_id"),
                hybrid_odr_collection_id=submission_params.get("hybrid_odr_collection_id"),
                hybrid_odr_source_mode=submission_params.get("hybrid_odr_source_mode"),
                # The server-side collection allowlist is intentionally not
                # persisted in the research row: it is deployment config,
                # not a user-controlled input. Re-resolve it in the worker.
